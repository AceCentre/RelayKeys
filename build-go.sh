...
echo "  macOS arm64..."
CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags "$LDFLAGS" -o $OUTDIR/relaykeys-daemon-darwin-arm64 ./cmd/relaykeys-daemon
CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags "$LDFLAGS" -o $OUTDIR/relaykeys-cli-darwin-arm64 ./cmd/relaykeys-cli

echo "  macOS amd64..."
CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -ldflags "$LDFLAGS" -o $OUTDIR/relaykeys-daemon-darwin-amd64 ./cmd/relaykeys-daemon
