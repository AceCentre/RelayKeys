    ...
    if "result" not in ret:
        logging.error(
            "keyevent ({}, {}, {}) response error: {}".format(
                key, modifiers, isdown, ret.get("error", "undefined")
            )
        )
        ...
        logging.info(
            "keyevent ({}, {}, {}) response: {}".format(
                key, modifiers, isdown, ret["result"]
            )
        )
    ...
    if "result" not in ret:
        logging.error(
            "mousemove ({}, {}) response error: {}".format(
                right, down, ret.get("error", "undefined")
            )
        )
        raise CommandErrorResponse()
    else:
        logging.info(
            "mousemove ({}, {}) response: {}".format(right, down, ret["result"])
        )
    ...
    if "result" not in ret:
        logging.error(
            "mousebutton ({}, {}) response error: {}".format(
                btn, behavior, ret.get("error", "undefined")
            )
        )
        raise CommandErrorResponse()
    else:
        logging.info(
            "mousebutton ({}, {}) response: {}".format(btn, behavior, ret["result"])
        )
    ...
    if "result" not in ret:
        logging.error(
            "devicecommand ({}) response error: {}".format(
                devcommand, ret.get("error", "undefined")
            )
        )
        raise CommandErrorResponse()
    else:
        logging.info(
            "devicecommand ({}) response : {}".format(devcommand, ret["result"])
        )
        if notify:
    ...
    if "result" not in ret:
        logging.error(
            "daemoncommand ({}) response error: {}".format(
                command, ret.get("error", "undefined")
            )
        )
        raise CommandErrorResponse()
    else:
