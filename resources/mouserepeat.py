...
parser.add_argument('--movex', '-x', dest='movex', default=0,
                    help='move in x pixels right')
parser.add_argument('--movey', '-y', dest='movey', default=0,
                    help='move in x pixels down')
...
def moveMouse(movex, movey, keepRunning=True):
    while keepRunning:
        ...
        print(fileargs)
        subprocess.call(
            ["C:\Program Files (x86)\Ace Centre\RelayKeys\\relaykeys-cli.exe", fileargs], shell=True)
